 ...
 */
static uint8_t cl_device_extension_available(const cl_device_id device, const char *const required_extension_name) {
 ...
 * @return `CL_TRUE` if supported, else `CL_FALSE`
 */
static uint8_t cl_device_feature_available(const ComputeContext *const ctx, const char *const required_feature_name) {
 ...
 * @return `CL_TRUE` if supported, else `CL_FALSE`
 */
static uint8_t cl_image2d_format_available(
